	...
	try {
      ...
      console.log("team is ___________________________" , team.teamName);
			...
			if (team.registeredPlayers.length > 0) {
				team.registeredPlayers.forEach((elem) => {
					let id3;
					let foundPlayer = cleanPlayersList.filter((player) => {
						return player.email === elem.player;
					});
					if (foundPlayer[0]) {id3 = foundPlayer[0]._id};
          if (id3) filteredArray3.push(id3)
          console.log("filtered arr ========" , filteredArray3);
				});			
      }
      team.registeredPlayers = filteredArray3;
      console.log("TEAM LIST ++++++++++++", team.registeredPlayers);
    });
